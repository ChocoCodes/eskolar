...
import { mapRecommendMatch, mapStatus } from '@/lib/utils';
        ...
        <Card className='w-[48%] flex flex-col justify-between px-4 shadow-md hover:-translate-y-1 hover:shadow-lg transition-transform duration-200'>
            ...
            <CardContent className='flex flex-col gap-2'>
                <div className="flex w-full justify-between font-semibold">
                    ...
                    <p className={`flex items-center text-xs px-4 rounded-full text-white ${mapStatus[scholarship.status.toLowerCase()]}`}>{ scholarship.status.toUpperCase() }</p>
                </div>
            ...
            <CardFooter className="flex justify-between w-full">
                <Link 
                    href={`/discover/${scholarship.id}`} 
                    className="text-dark-grayish-blue hover:underline"
                >
                    See Details &rarr;
                </Link>
                <div className={`flex gap-2 items-center border-2 py-1 px-2 rounded-sm text-sm ${ mapRecommendMatch[scholarship.match?.toLowerCase() || ""] || "bg-gray-300" }`}>
                    <FaThumbsUp />
